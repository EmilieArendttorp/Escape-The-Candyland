...
public class RandomFlyer : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (body.transform.position.y < yMinMax.x + 1f ||
            body.transform.position.y > yMinMax.y - 1f)
        {
            if (body.transform.position.y < yMinMax.x + 1f) rotateTarget.y = 1f; else rotateTarget.y = -1f;
        }
